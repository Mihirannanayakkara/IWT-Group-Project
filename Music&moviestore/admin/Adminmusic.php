...
<a href="../project/indexreg.html" >Log out</a>
  ...
  <div id="main-content" class="container allContent-section py-4">
    <div class="all">
      <center><h2 class="cusmuscmov"><i class="fa fa-music" style="font-size:100px"></i> Music</h2>
      <div class="sea">
      <form method="GET" action="Adminmusic.php" class="search">
            <input type="text" name="search" placeholder="Search Name or Category...">
            <button type="submit"><i class="fa fa-search"></i></button>
        </form>
        </div>
      <a href="Musicform.php"><button class="addbtn" ><i class="fa fa-upload"></i></button></a><br><br>
    ...
    <tbody>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td><i class='fa fa-usd'>" . $row["Price"] . "</i></td>";
                echo "<td>" . $row["Description"] . "</td>";
                echo "<td><a href='javascript:void(0);' onclick='confirmDelete(" . $row["MusicID"] . ");'><i class='fa fa-trash'></i></a></td>";
                echo "<td><a href='Editmusic.php?id=" . $row["MusicID"] . "'><i class='fa fa-edit'></a></i></td>";
                echo "</tr>";
            }
        } else {
...
<div class="pagination">
                ...
                $totalCountQuery = "SELECT COUNT(*) AS total FROM music";
                ...
                for ($i = 1; $i <= $totalPages; $i++) {
                    ...
                    echo "<a href='Adminmusic.php?page=$i' class='$isActive'>$i</a>";
                }
                ?>
            </div>
            <script>
                function confirmDelete(MusicID) {
                    if (confirm('Are you sure you want to delete this record?')) {
                        window.location.href = 'Deletemusic.php?id=' + MusicID;
                    }
                }
            </script>
        </div>
